...
src/
  ...
  store.js           # 数据层（Supabase CRUD + 行情刷新）
...
**V1.5（数据增强）：** ✅
- 接入行情数据API（腾讯财经 qt.gtimg.cn，CORS 开放，A股+港股统一接口）
- 页面加载时自动刷新现价，手动刷新按钮

**V1.6（体验增强）：** ✅
- 负成本价收益计算修复（翻倍卖出后剩余持仓成本为负的场景）
- 股票代码自动关联名称（输入代码后自动查询腾讯财经API填充名称，支持A股+港股）
- 港股实时行情（A股格式 s_sh/s_sz + 6位代码，港股格式 r_hk + 5位代码）
- 个股详情页桌面端左右布局（左：研究摘要+追踪锚，右：思考时间线）
- 首页桌面端左右布局（左：股票列表，右：统计卡片+持仓分布饼图+个股盈亏柱状图）
- 引入 recharts 图表库
- 页面容器从 max-w-3xl 扩展至 max-w-6xl，左右布局真正铺开
