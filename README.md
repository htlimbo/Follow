...
![组合总览](docs/screenshots/login.png)
...
- **买入/卖出快捷入口**：一键记录交易（价格 + 股数），自动计算加权成本价和持仓更新，全部卖出自动清仓
- **快照存证**：记录想法时一键保存当前价格和盈亏数据
- **逻辑标签**：#宏观驱动 #基本面反转 #情绪博弈 #套利 + 自定义标签
- **重复防重**：添加股票时自动检查，同一代码不允许重复添加

![个股研究卡片](docs/screenshots/detail.png)

### 阶段复盘

定期回顾你的操作和思考质量，不只是看盈亏数字。

- **判断记分卡**：回顾选定时间段内的买入、卖出、调仓操作，四维度标记——逻辑证实 / 待验证 / 逻辑证伪 / 意外因素，统计兑现率
- **操作纪律审计**：记录频率热力图（GitHub 风格）、追踪锚更新率、逾期未更新提醒
- **复盘笔记**：自由文本记录阶段性总结，按时间段存档
- **"回到过去"反思区**：如果回到这段时间的开始，带着现在的认知，你会做出什么不同的决定？

![阶段复盘](docs/screenshots/review.png)

### 写作日志

与个股无关的独立写作空间，记录投资心得、阅读笔记、策略思考。

- 博客式写作体验：标题 + 正文 + 标签，沉浸式编辑
- 预设标签（宏观思考、策略复盘、市场情绪、读书笔记、交易纪律、生活感悟）+ 自定义标签
- 按标签筛选，卡片式列表展示
- 支持编辑和删除

![写作日志](docs/screenshots/journal.png)
